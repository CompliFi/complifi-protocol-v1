...
const DerivativeSpecificationRegistry = artifacts.require(
...
contract("VaultFactory", (accounts) => {
  ...
  it("...should be initialised.", async () => {
    assert.equal(
      await vaultFactory.collateralTokenRegistry.call(),
      (await CollateralTokenRegistry.deployed()).address
    );
    assert.equal(
      await vaultFactory.collateralSplitRegistry.call(),
      (await CollateralSplitRegistry.deployed()).address
    );
    assert.equal(
      await vaultFactory.derivativeSpecificationRegistry.call(),
      (await DerivativeSpecificationRegistry.deployed()).address
    );
    assert.equal(
      await vaultFactory.oracleRegistry.call(),
      (await OracleRegistry.deployed()).address
    );
    assert.equal(
      await vaultFactory.oracleIteratorRegistry.call(),
      (await OracleIteratorRegistry.deployed()).address
    );
    assert.equal(
      await vaultFactory.tokenBuilder.call(),
      (await TokenBuilder.deployed()).address
    );
