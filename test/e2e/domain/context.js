...
const initVaultFactory = async () => {
  get["vaultFactory"] = await VaultFactory.at((await VaultFactoryProxy.deployed()).address);
}
...
const createVaultBy = async (symbol) => {
  const vaultCreated = Math.floor(Date.now() / 1000);
  const vaultFactory = get["vaultFactory"];
  await vaultFactory.createVault(web3.utils.keccak256(symbol), vaultCreated);
  ...
  get["vault"] = vault;
  get["vaultCreated"] = vaultCreated;

  get["primaryToken"] = await ERC20PresetMinter.at(await vault.primaryToken.call());
  get["complementToken"] = await ERC20PresetMinter.at(await vault.complementToken.call());

  get["collateralSplit"] = await StubCollateralSplit.at(await vault.collateralSplit.call());
};
...
const calcConversions = async (startValue, endValue) => {
  const collectedCollateral = await get["stubToken"].balanceOf.call(get["vault"].address);
  ...
  const primaryConversion = Math.trunc(collectedCollateral * w / mintedPrimaryTokenAmount);
  const complementConversion = Math.trunc(collectedCollateral * (FRACTION_MULTIPLIER - w) / mintedPrimaryTokenAmount);
...
const issueCollateralsTo = async (user, value) => {
  ...
  assert.equal((await get["stubToken"].balanceOf.call(user)).toString(), currentBalance.add(value).toString());

  const currentAllowance = await get["stubToken"].allowance.call(user, get["vault"].address);
  await get["stubToken"].approve(get["vault"].address, value, {from: user});
  assert.equal((await get["stubToken"].allowance.call(user, get["vault"].address)).toString(), currentAllowance.add(value).toString());
}

const mintFor = async (user, value) => {
  const currentBalance = await get["stubToken"].balanceOf.call(get["vault"].address);
  await get["vault"].mint(value, {from: user});
  if(await get["vault"].state.call() === STAGE.Minting) {
    assert.equal((await get["stubToken"].balanceOf.call(get["vault"].address)).toString(), toBN(value).add(currentBalance).toString());
  }
};

const approveTokensFor = async (user, primaryValue, complementValue) => {
  await get["primaryToken"].approve(get["vault"].address, primaryValue.toString(), {from: user});
  await get["complementToken"].approve(get["vault"].address, complementValue.toString(), {from: user});
...
const denominate = async (value) => {
  ...
  const primaryNominalValue = await get["specification"].primaryNominalValue.call();
  const complementNominalValue = await get["specification"].complementNominalValue.call();
...
const calcRedeemedAmount = (value, primaryConversion, complementConversion) => {
  return Math.trunc((primaryConversion * value + complementConversion * 0) / FRACTION_MULTIPLIER);
}
...
const checkFees = async (value, func) => {
  ...
  const protocolFee = await calcProtocolFee(value);
  assert.equal((await getCollateralBalance(author)).toString(), authorBalance.add(await calcAuthorFee(toBN(value))).toString());
  assert.equal((await getCollateralBalance(protocolWallet)).toString(), protocolBalance.add(protocolFee).toString());
}
...
const checkTokensAreEqualFor = async (user, primaryAmount, complementAmount) => {
  assert.equal((await get["primaryToken"].balanceOf.call(user)).toString(), primaryAmount.toString());
  assert.equal((await get["complementToken"].balanceOf.call(user)).toString(), complementAmount.toString());
};

const checkTokensAllowanceAreEqualFor = async (user, primaryAmount, complementAmount) => {
  assert.equal((await get["primaryToken"].allowance.call(user, get["vault"].address)).toString(), primaryAmount.toString());
